...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (supportSubmitBtn && supportForm) {
        supportSubmitBtn.addEventListener('click', async (e) => {
            ...
            // ارسال تیکت به سرور
            const res = await fetch(`${API_BASE}?action=save_ticket`, {
                method: "POST",
    ...
    if (deleteSelectedTicketsBtn) {
        deleteSelectedTicketsBtn.addEventListener('click', async () => {
            ...
            const res = await fetch(`${API_BASE}?action=delete_tickets`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    async function renderSupportTicketsFromServer() {
        ...
        const res = await fetch(`${API_BASE}?action=get_tickets&user_id=${userId}`);
        const data = await res.json();
    ...
    async function fetchDiscountFromServer(code) {
        try {
            const res = await fetch(`${API_BASE}?action=get_discount&code=` + encodeURIComponent(code));
            const data = await res.json();
            return data;
        } catch (e) {
            return { status: "error" };
    ...
    async function openUserInfoSection(mode = 'profile') {
        ...
        try {
            const response = await fetch(`${API_BASE}?action=get_user&id=${userId}`);
        ...
        fetch(`${API_BASE}?action=get_orders&user_id=${userId}`)
            .then(res => res.json())
            .then(orders => {
                if (!orders.length) {
    ...
    if (userInfoForm) {
        userInfoForm.addEventListener('submit', async function (e) {
            ...
            let response = await fetch(`${API_BASE}?action=update_user`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    id: userId,
            ...
            if (updated.phone) {
                await fetch(`${API_BASE}?action=update_phone`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        user_id: userId,
    ...
    if (trackingSearchBtn && trackingInput) {
        trackingSearchBtn.addEventListener('click', async function (e) {
            ...
            // درخواست به سرور
            const res = await fetch(`${API_BASE}?action=track_order&code=${code}`);
            const data = await res.json();
    ...
    async function loadUserReviews() {
        ...
        const res = await fetch(`${API_BASE}?action=get_reviews&user_id=${userId}`);
        const data = await res.json();
    ...
    saveAddressBtn.addEventListener("click", async () => {
        ...
        const res = await fetch(`${API_BASE}?action=save_address`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                user_id: userId,
...
`).join('')}
    ...
    async function loadProductsFromServer() {
        try {
            const res = await fetch(`${API_BASE}?action=get_products`);
            const data = await res.json();
